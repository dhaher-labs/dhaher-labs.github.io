...
export const articles: Article[] = [
  ...
  {
    id: 'automated-workflow-design-patterns',
    title: 'Automated Workflow Design Patterns',
    summary: 'KALEN demonstrates practical design patterns for building automated workflow orchestration systems, from DAG-based task scheduling to smart retry and recovery strategies.',
    content: [
      'Automated workflows are sequences of tasks that execute with minimal human intervention, adapting to conditions and recovering from failures automatically. KALEN provides a framework for defining, executing, and monitoring such workflows.',
      'The core abstraction in KALEN is the workflow graph — a directed acyclic graph (DAG) where nodes represent tasks and edges represent dependencies. Tasks execute only when all their upstream dependencies have completed successfully. This DAG-based approach naturally handles parallelism: independent tasks run concurrently while dependent tasks wait for their prerequisites.',
      'KALEN implements several execution strategies. The simplest is the static DAG, where the workflow structure is defined upfront and does not change during execution. More advanced is the dynamic DAG, where the workflow structure can be modified at runtime based on task outputs — for example, a data validation task might add additional processing steps if it detects anomalies.',
      'Error handling is a critical concern in automated workflows. KALEN provides configurable retry policies with exponential backoff, circuit breakers for external service calls, and dead letter queues for tasks that cannot be completed. When a task fails, the system can either retry it, skip it with a default value, or fail the entire workflow — depending on the task criticality configuration.',
    ...
    readTime: '7 min read',
    featured: false,
    tags: ['Workflows', 'Automated', 'KALEN', 'DAG', 'TypeScript', 'Orchestration'],
    aiGenerated: true,
  },
  {
